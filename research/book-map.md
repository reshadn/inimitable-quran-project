# Book map and edition strategy

Working titles remain provisional. All editions share evidence IDs and an underlying source ledger; shortening an argument must not strengthen its conclusion.

## Flagship: The Qur’an Examined

Target: 90,000–120,000 words. General readers, including skeptical readers, with Arabic explained in place.

| Chapter | Main question | Required evidence or artifact | Current status |
|---|---|---|---|
| Opening | How can a reader investigate fairly? | Stated inference levels | Drafted |
| 1. The challenge | What does the text actually claim? | 2:23–24, 10:38, 11:13, 17:88, 52:33–34 and tafsir | Drafted in edition 0.2; independent review pending |
| 2. The distance between claims | How do distinctiveness, excellence, and inimitability differ? | Philosophical argument map | Drafted in edition 0.2; independent review pending |
| 3. A history of the question | Where do classical theories agree and differ? | Naẓm, rhetoric, ṣarfa, theology | Selected-arguments chapter drafted in edition 0.2; wider history pending |
| 4. The head ablaze | How does grammar change an image? | 19:2–6; al-Jurjānī and competing analyses | Sample drafted |
| 5. Economy and implication | What can fewer words communicate? | Annotated passages and counterexamples | Planned |
| 6. Sound and meaning | What belongs to the text and what to performance? | Phonological and recitation studies | Planned |
| 7. Shifts of address | When does perspective advance discourse? | Whole-context linguistic analysis | Planned |
| 8. Time and mutual responsibility | How does a short surah make an argument? | Full surah 103 and classical readings | Sample drafted |
| 9. Other short compositions | Do proposed effects recur? | Surahs 1, 93, 108, 112, 114 | Planned |
| 10. Prayer and dialogue | What does context add to a phrase? | Complete surah 19 | Planned |
| 11. Narrative | How do arrangement and retelling function? | Complete surah 12; responsible comparators | Planned |
| 12. Larger structures | Which structural proposals survive alternatives? | Surahs 36, 55, 67, 75; independent boundaries | Planned |
| 13. Strong alternatives | What constitutes a fair comparison? | Corpus selection and attribution audit | Criteria sketched; final protocol pending |
| 14. Reader judgments | What do experts and other readers agree on? | Human studies and recognition checks | Not conducted |
| 15. Mathematical description | What do the measurements actually show? | Reproducible descriptive and comparative analyses | Descriptive pilot and methods chapter drafted; proposed studies not conducted |
| 16. Language through translation | What transfers and what needs explanation? | Multiple renderings and reader study | Planned |
| 17. Historical circumstances | Which composition premises have evidence? | Independent historical source criticism | Bibliography begun |
| 18. Transmission and readings | What textual state is being analyzed? | Manuscripts, readings, source-specific claims | Baseline documented |
| 19. The strongest human explanation | What explanatory work can it do? | Steelmanned competing account | Planned |
| 20. The case for revelation | What does the cumulative evidence warrant? | Verified findings and explicit philosophical premises | Not yet written |

The selected twelve surahs are editorial cases. They are not a random sample and cannot support a claim about typical performance without a separate sampling design.

## Academic monograph

Target: 120,000–160,000 words excluding references. Expand the classical history, competing readings, philological notes, source-critical history, methods, and full results. Preserve positive and null results. Appendices contain corpus manifests, annotation manuals, hypotheses, model specifications, robustness analyses, and the final claim ledger. No final conclusion is drafted before the evidence it relies on exists.

## Illustrated introduction

Target: 40,000–60,000 words. Adapt the strongest reviewed material from the flagship. Use aligned Arabic and English, word-relation diagrams, whole-surah maps, and annotated comparisons. Move technical detail to stable companion links while retaining essential qualifications in the page itself.

## Novelty register

Candidate original contributions: controlled changes to expression with matched controls; joint profiles of literary objectives; phrase-to-surah functional tracing; uncertainty-aware comparison maps; and reader-accessible evidence trails. These are proposed contributions. A literature search and expert review must precede any claim to have invented a new method.

The mathematical and cognitive studies will be research articles as well as book inputs. Articles are optional publication formats, not authorized submissions.
