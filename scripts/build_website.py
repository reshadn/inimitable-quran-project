...
# Immutable published audio revision. This host supports byte ranges for seeking.
AUDIO_BASE = 'https://raw.githubusercontent.com/reshadn/inimitable-quran-project/5600c13334804e46d5519a225468d2c3ae783646/audio/'
...
def main():
    ...
    subprocess.run([sys.executable, str(ROOT/'scripts/build_bibliography.py')], check=True)
    ...
    entries = edition['chapters']
    chapters = [ROOT / entry['file'] for entry in entries]
    assert all(path.exists() for path in chapters), 'A chapter in the reading order is missing'
    assert len({entry['slug'] for entry in entries}) == len(entries), 'Duplicate chapter slug'
    audio_by_slug = {track['slug']: track for track in meta['chapters']}
    tracks = [dict(audio_by_slug[entry['slug']], label=entry['label']) for entry in entries if entry['slug'] in audio_by_slug]
    for track in tracks:
        track['stream_url'] = track.get('stream_url') or AUDIO_BASE + track['filename']
    cards_list = []
    for entry in entries:
        track = audio_by_slug.get(entry['slug'])
        duration = track['duration_label'] + ' audio' if track else 'Text edition'
        listen = f'<a href="/listen/#{entry["slug"]}">Listen</a>' if track else ''
        new = ' · New in 0.2' if entry.get('new') else ''
        cards_list.append(f'''<article class="card"><p class="eyebrow">{html.escape(entry['label'])}</p><h3>{html.escape(entry['title'])}</h3><p>{html.escape(entry['description'])}</p><div class="meta">{duration} · Working draft{new}</div><div class="card-links"><a href="/read/{entry['slug']}/">Read the chapter →</a>{listen}</div></article>''')
    cards = '<div class="cards">' + ''.join(cards_list) + '</div>'
    words = sum(len(path.read_text().split()) for path in chapters)
    notice = f'<p class="notice">Expanded research draft 0.2: an opening essay and {len(entries)-1} chapters, approximately {round(words/100)*100:,} words including notes. Four chapters are new in this edition. The full book and proposed studies remain in development; independent scholarly review is pending.</p>'
    ...
    urls = [BASE+'/',*[BASE+p for _,p in NAV],*[BASE+'/read/'+entry['slug']+'/' for entry in entries]]
